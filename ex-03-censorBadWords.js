...
function censorBadWords(arrayP, palabras) {
// console.log(arrayP);
...
var palabrassplit = palabras.split(' ')
  ...
  for (var i = 0; i < palabrassplit.length; i++) {
    newpalabras = palabrassplit[i]
    // console.log(newpalabras);

    if (arrayP.indexOf(newpalabras) !== -1) {
      palabrassplit[i] = '****'
      // console.log(palabrassplit[i] = '****');
    }
  }

  console.log(palabrassplit.join(' '));
  return palabrassplit.join(' ')
}
